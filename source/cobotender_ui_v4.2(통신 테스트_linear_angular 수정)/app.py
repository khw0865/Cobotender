...
class DoosanRosBridge(Node if ROS_CORE_AVAILABLE else object):
    def __init__(self):
        ...
        if DSR_VELX_AVAILABLE:
            ...
            self.create_timer(0.2, self._request_current_velx)
        else:
    ...
    def _request_current_velx(self):
        if self.velx_client is None or self.velx_request_pending:
            return
        if not self.velx_client.service_is_ready():
            self.velx_client.wait_for_service(timeout_sec=0.0)
            return
