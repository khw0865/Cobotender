...
- `app.py`
  - `/api/order` 처리 중 `category == 'snack'`인 항목을 `staff_requests`에 추가하도록 변경했습니다.
  - 저장 형식: `안주 주문: <메뉴명> x<수량>`
- `static/js/admin.js`
  - 기존 요구사항 팝업에 안주 주문 아이콘(`🍽️`) 표시를 추가했습니다.
- 기존 요구사항(`물`, `냅킨`, `직원호출`) 팝업 동작은 유지했습니다.


## Admin command panel update
...
추가 기능:
- 관리자 UI에서 PAUSE / RESUME / CANCEL / RECOVER 명령 전송
- `/ui/admin_control` `std_msgs/String` publisher 추가
- `/robot/admin_bridge_status` `std_msgs/String` subscriber 추가
- 안전상태/Recovery/Bridge 연결 상태 카드 추가
- 위험 명령 확인 팝업 추가
