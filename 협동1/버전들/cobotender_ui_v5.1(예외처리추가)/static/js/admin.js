...
const COMMAND_CONFIRM = {
  cancel: '현재 주문을 취소하고 로봇 작업을 중단하시겠습니까?',
  estop: '비상정지를 실행하시겠습니까? 로봇이 즉시 정지합니다.',
  estop_reset: '비상정지를 해제하고 홈 포지션 복귀를 요청하시겠습니까?',
  recover: '로봇 주변 안전을 확인했습니까? 안전모드 Recovery 명령을 전송합니다.'
};

async function sendRobotCommand(command){
  const confirmMessage = COMMAND_CONFIRM[command];
