...
- 고객용 UI에서 `snack` 카테고리 항목을 주문하면 `staff_requests` 테이블에 `안주 주문: 메뉴명 x수량` 형식으로 기록됩니다.
- 관리자용 UI는 기존 요구사항 팝업(`/api/staff_requests`)을 그대로 사용해 안주 주문도 함께 표시합니다.
- 안주 주문은 로봇 제조 명령 대상이 아니므로 `/ui/menu_command`로는 전송되지 않습니다.


## Admin control UI bridge 반영

이번 버전은 관리자 대시보드의 로봇 명령 패널을 확장했습니다.

- `일시정지` 버튼: `/ui/admin_control`에 `PAUSE` 발행
- `재개` 버튼: `/ui/admin_control`에 `RESUME` 발행
- `주문취소` 버튼: `/ui/admin_control`에 `CANCEL` 발행
- `비상정지` 버튼: `/ui/admin_control`에 `ESTOP` 발행, 기존 호환용으로 `/ui/emergency_stop=True`도 함께 발행
- `비상정지 해제 + 홈복귀` 버튼: `/ui/admin_control`에 `ESTOP_RELEASE_HOME` 발행, 기존 호환용으로 `/ui/emergency_stop=False`도 함께 발행
- `안전모드 Recovery` 버튼: `/ui/admin_control`에 `RECOVER` 발행

별도 중계 노드 `bartender_admin_control_bridge.py`를 실행하면 `/ui/admin_control` 명령을 기존 제어코드용 토픽으로 분배할 수 있습니다.
관리자 UI는 `/robot/admin_bridge_status`를 구독해 protective stop, e-stop, safe off, recovery 대기 상태와 bridge 로그를 표시합니다.
