...
TASK_CONTROL_TOPIC = f"/{ROBOT_ID}/task_control"  # std_msgs/String: PAUSE, RESUME, HOME_RETURN
...
VALID_COMMANDS = {
    "PAUSE",
    "RESUME",
    # CANCEL is kept for backward compatibility, but the latest admin UI no longer exposes it.
    "CANCEL",
    "ESTOP",
    "ESTOP_RELEASE",
    "HOME_RETURN",
    # Legacy combined command kept for older UI packages.
    "ESTOP_RELEASE_HOME",
    "RECOVER",
}
...
class BartenderAdminControlBridge(Node):
    ...
    def _admin_command_cb(self, msg: String):
        ...
        if command in ("PAUSE", "RESUME", "CANCEL", "HOME_RETURN"):
            out = String()
            out.data = command
        ...
        if command == "ESTOP_RELEASE_HOME":
            # Legacy combined command: release first, then request home return via task_control.
            out = Bool()
            out.data = False
            self.estop_pub.publish(out)
            task = String()
            task.data = "HOME_RETURN"
            self.task_pub.publish(task)
            self._add_log("OK", "legacy emergency release + home published")
            return
